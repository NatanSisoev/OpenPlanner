...
async function runAgentForPlanJson(opts: AgentPlanJsonRunOptions): Promise<Plan> {
  ...
  traceEvent(tid, "agentPlanJson.config", { agentPath, resolvedAgent, cwd, timeoutMs, maxStdoutChars });
  ...
  if (exitCode !== 0) {
    ...
    traceEvent(tid, "agentPlanJson.parse_skipped", { reason: "nonzero_exit", tailHead: tail.slice(0, 400) });
    throw new AgentCliError(
  ...
  try {
    ...
    traceEvent(tid, "agentPlanJson.extract_json", { jsonTextChars: jsonText.length });
    parsed = JSON.parse(jsonText);
  } catch (e) {
    const msg = e instanceof Error ? e.message : String(e);
    traceEvent(tid, "agentPlanJson.parse_json_fail", { message: msg, stdoutHead: stdout.slice(0, 400) });
    throw new AgentCliError(
  ...
  try {
    ...
    traceEvent(tid, "agentPlanJson.validate_ok", { planId: plan.id, title: plan.title });
    return plan;
  } catch (e) {
    const msg = e instanceof Error ? e.message : String(e);
...
export async function resyncPlanFromCurrentWorkspace(opts: ResyncPlanFromCliOptions): Promise<Plan> {
  ...
  traceEvent(tid, "resyncPlan.exit", { returnedPlanId: plan.id, returnedPhases: plan.phases.length });
  return plan;
}
